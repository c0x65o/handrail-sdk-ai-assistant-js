...
All React styled presets now use `StandardChatComposer`, also exported from `@handrail/ai-assistant/react/styled` for custom application shells. Flutter consumers use the companion [`handrail_ai_widgets`](https://github.com/c0x65o/handrail-sdk-ai-assistant-flutter/blob/main/packages/handrail_ai_widgets/README.md) package. The composer follows the supplied two-row reference: multiline text at the upper left, Add and shield at the lower left, dictation and green circular Send at the lower right. The send arrow becomes Stop while an active operation has cancellation available.
...
Hosts retain public HTTPS Git dependencies pinned to full committed SDK revisions and matching lockfiles. Candidate shared UI changes require an authorized SDK revision update through normal installation/build. Host theming uses the supported composer/workspace properties; the old compact compatibility styles and bespoke composer workarounds have been removed. See [the consolidation ledger](shared-assistant-consolidation.md) for consumer evidence and remaining work.
