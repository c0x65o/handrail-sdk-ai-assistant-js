...
`recoverPending(limit)` starts at most `limit` eligible turns, subject to the shared recovery execution cap. Denials and live leases do not exhaust that work limit. Each database page is bounded, but reaching eligible work can require authorization checks across the tenant's pending backlog. The high-level scheduler now uses `recoverPendingPage` with fresh authentication for each 25-candidate batch; see [recovery discovery](recovery-discovery.md) for metadata-only reads, the additive schema migration, concurrency limits and shutdown behavior.
