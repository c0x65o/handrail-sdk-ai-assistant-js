...
export async function deletePostgresConversationHistory(input: DeletePostgresConversationHistoryInput) {
  ...
  return input.client.transaction(async client => {
    ...
    const approvals = await client.query(`SELECT 1 FROM handrail_ai_approvals WHERE tenant_id=$1
      AND group_id=$2 AND payload->>'status'='pending' LIMIT 1`, [input.tenantId, input.conversationId]);
    ...
    const providers = await client.query(`SELECT 1 FROM handrail_ai_documents WHERE tenant_id=$1
      ...
      ) IS NOT TRUE LIMIT 1`, [input.tenantId, input.conversationId]);
    ...
    for (;;) {
      ...
      [input.tenantId, input.conversationId, after?.scope_id ?? null, after?.record_id ?? null]);
      if (!calls.rows.length) break;
      for (const row of calls.rows) {
        try {
          const call = await new PostgresRealtimeCallStore(persistence, input.tenantId, row.scope_id).get(row.record_id);
          if (!call || call.conversationId !== input.conversationId || call.status !== 'ended') throw new PostgresConversationDeletionBlockedError();
      ...
      const unfinished = await client.query(`SELECT 1 FROM handrail_ai_documents WHERE tenant_id=$1
        ...
        AND COALESCE(payload->>'status','') NOT IN ('completed','failed') LIMIT 1`, [input.tenantId, scopes]);
      if (unfinished.rows.length) throw new PostgresConversationDeletionBlockedError();
      // Read acknowledgements are presentation state; bounded call/tool outcome
      // receipts remain as audit evidence, with no transcript or business result.
      await client.query(`DELETE FROM handrail_ai_documents WHERE tenant_id=$1
        AND kind='realtime_activity_read' AND scope_id=ANY($2::text[])`, [input.tenantId, scopes]);
      if (calls.rows.length < 100) break;
      after = calls.rows.at(-1);
    }
    const history = await replayConversation({
      eventStore: new PostgresConversationEventStore(persistence, input.tenantId),
      conversationId: input.conversationId, checkpointPolicy: false,
    });
    try {
