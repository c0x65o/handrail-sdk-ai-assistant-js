...
try {
  ...
  for (const width of [390, 1280]) {
    ...
    await page.getByText("Thinking…", { exact: true }).waitFor({ timeout: 3000 });
    ...
    await page.getByText("Writing response…", { exact: true }).waitFor();
    ...
    const geometry = await page.evaluate(() => {
      ...
      return { width: innerWidth, scroll: document.documentElement.scrollWidth,
        ...
        inline: transcript.contains(card), composerBottom: composer.getBoundingClientRect().bottom };
    });
    ...
    assert.notEqual(await group.getAttribute("open"), null);
    assert.equal(await group.getAttribute("data-active"), "false");
