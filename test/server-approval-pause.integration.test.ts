...
// One embedded PostgreSQL VM per file; independent tenants keep cases isolated.
// Seven separately created WASM instances previously peaked near 2 GiB.
...
beforeAll(async () => { await persistence.persistence.migrate(); }, 15_000);
...
it.each(['confirmed', 'rejected'] as const)('HTTP decisions resume an external %s proposal after restart without reopening its provider', async status => {
  ...
  const runtime = async (authorize: () => void | Promise<void>) => createAssistantToolRuntime({ context: testContext,
    events: bundle.events, proposalStore: bundle.approvals, authorizeLocation: authorize,
    application: await createAiApplication({ installContext: testContext, plugins: [plugin], policy: () => ({ outcome: 'allow' }),
  ...
  const create = () => createHandrailAssistant({ id: 'external', authorize: () => testContext, persistence, automaticTitles: false,
...
it.each([['confirmed', 'complete'], ['rejected', 'complete'], ['confirmed', 'running'], ['confirmed', 'none'], ['rejected', 'none']] as const)('rests across comments and restart, then resumes %s once (comment=%s)', async (decision, commentMode) => {
  ...
  const create = () => createHandrailAssistant({ id: 'test', authorize: () => testContext, persistence, tools: [plugin], diagnostics, automaticTitles: false,
  ...
  try {
    ...
    expect((await bundle.approvals.get({ permissionContext: testContext, proposalId: proposal.proposal_id }))?.status).toBe('pending');
